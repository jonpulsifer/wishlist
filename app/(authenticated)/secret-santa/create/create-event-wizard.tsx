...
export function CreateEventWizard({ families }: { families: FamilyOption[] }) {
  ...
  // Navigation hides this route rather than unmounting it, so a finished
  // wizard would re-open on step 3 holding the previous event. The cleanup
  // runs when the route is hidden; coming back always starts fresh.
  useLayoutEffect(
    () => () => {
      setStep(1);
      setSelectedUsers([]);
  ...
  return (
    <>
      {/* Below sm only the numbered circles fit; the step's own card title
          names where you are. */}
      <div className="flex justify-between items-center mb-4">
        <div className="flex gap-2">
          <div
            className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${step >= 1 ? 'bg-primary text-primary-foreground' : 'border bg-background'}`}
          >
        ...
        <div className="flex gap-2">
          <div
            className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${step >= 2 ? 'bg-primary text-primary-foreground' : 'border bg-background'}`}
          >
        ...
        <div className="flex gap-2">
          <div
            className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${step >= 3 ? 'bg-primary text-primary-foreground' : 'border bg-background'}`}
          >
            ...
            <Form {...form}>
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <FormDescription>
                    Everyone drawn comes from this family, so each santa can see
                    what their person asked for.
                  </FormDescription>
                </div>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800 p-4 rounded-md">
                <p className="text-sm">
                  <strong>Important:</strong> Once you confirm and assign Secret
                  Santas, the assignments cannot be changed. Make sure all
                  participants are correct before proceeding.
                </p>
              </div>
            </div>
          </CardContent>
